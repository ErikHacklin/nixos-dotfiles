{ config, pkgs, lib, ... }: {
  home.activation.setVarietyWallpaperScript = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    mkdir -p "$HOME/.config/variety/scripts"
    cat > "$HOME/.config/variety/scripts/set_wallpaper" <<'EOF'
#!/usr/bin/env bash
...
exit 0
EOF
    chmod +x "$HOME/.config/variety/scripts/set_wallpaper"
  '';
}
